...
defineFeature(feature, (test) => {
  ...
  test('Waiting status, the remaining mines counter show the number of hidden mines, by default, 10', ({ given, then }) => {
    ...
    then(/^the remaining mines counter should be (\d+)$/, (minesNum: number) => {
  ...
  test('Waiting status, remaining clicking a cell, the game status should be playing, the button status show a happy face', ({ given, when, then }) => {
    ...
    given('the player loads the following mock data', (docString: string) => {
      coreSteps.setMockData(docString)
    })
    when(/^the player uncovers the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
  ...
  test('Waiting status, right clicking a cell, the game status should be playing', ({ given, when, then }) => {
    ...
    when(/^the player tags as mined the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
  ...
  test('Playing status, the remaining mines counter show the number of hidden mines', ({ given, when, then }) => {
    ...
    then(/^the remaining mines counter should show "(.*)"$/, (minesNum: number) => {
      expect(steps.checkMinesCounterValue(minesNum)).toBe(true)
    })
  })
  test('Playing status, the timer starts', ({ given, when, and, then }) => {
    ...
    and(/^the user waits "(.*)" seconds$/, (seconds: number, done) => {
      setTimeout(() => {
        done()
      }, seconds * 1000)
    })
  ...
  test('Untagging a cell as mine, the remaining mines counter increase', ({ given, and, when, then }) => {
    ...
    and(/^the player tags as mined the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
      coreSteps.tagCellAsMined(rowPosition, colPosition)
    })
    when(/^the player untags the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
      coreSteps.untagCell(rowPosition, colPosition)
    })
    then(/^the remaining mines counter should be "(.*)"$/, (minesNum: number) => {
      expect(steps.checkMinesCounterValue(minesNum)).toBe(true)
    })
  })
  ...
  test('Tagging a cell as inconclusive, the remaining mines counter remains equal', ({ given, when, then }) => {
    ...
    when(/^the player tags as inconclusive the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
  ...
  test('Change tag from inconclusive to mined, the remaining mines counter decrease', ({ given, and, when, then }) => {
    ...
    and(/^the player tags as inconclusive the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
      coreSteps.tagCellAsInconclusive(rowPosition, colPosition)
    })
    when(/^the player tags as mined the cell \("(.*)","(.*)"\)$/, (rowPosition: number, colPosition: number) => {
      coreSteps.tagCellAsMined(rowPosition, colPosition)
